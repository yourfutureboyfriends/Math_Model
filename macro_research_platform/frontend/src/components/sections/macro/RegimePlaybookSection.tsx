...
const DEFAULT_PLAYBOOKS: Record<string, RegimePlaybookData> = {
  'Expansion': {
    regime: 'Expansion',
    summary: 'Growth accelerating with manageable inflation. Risk-on: overweight equities and cyclicals.',
    assetAllocation: [
      { asset: 'Equities', bias: 'LONG', conviction: 'HIGH', sizing: '40-50%', rationale: 'Cyclicals, industrials, tech lead' },
      { asset: 'Credit', bias: 'LONG', conviction: 'MEDIUM', sizing: '15-20%', rationale: 'Spreads tight, carry favourable' },
      { asset: 'Commodities', bias: 'LONG', conviction: 'MEDIUM', sizing: '10-15%', rationale: 'Energy and base metals demand' },
      { asset: 'Treasuries', bias: 'SHORT', conviction: 'MEDIUM', sizing: '5-10%', rationale: 'Short-to-mid duration; rates firm' },
      { asset: 'Cash', bias: 'NEUTRAL', conviction: 'LOW', sizing: '5%', rationale: 'Opportunity reserve' },
    ],
    factorPreferences: [
      { factor: 'Momentum', preference: 'OVERWEIGHT', conviction: 'HIGH', rationale: 'Trend persistence strong' },
      { factor: 'Value', preference: 'OVERWEIGHT', conviction: 'MEDIUM', rationale: 'Cyclical value participates' },
      { factor: 'Size', preference: 'OVERWEIGHT', conviction: 'MEDIUM', rationale: 'Small caps geared to growth' },
      { factor: 'Quality', preference: 'NEUTRAL', conviction: 'MEDIUM', rationale: 'Balanced vs cyclical tilt' },
    ],
    riskGuidelines: [
      ...
      { rule: 'Credit Watch', priority: 'HIGH', description: 'Trim risk if HY spreads widen > 100bps' },
...
export function RegimePlaybookSection({ data, currentRegime }: Props) {
  ...
  // Capitalise first letter for DEFAULT_PLAYBOOKS lookup
  const regimeKey = resolvedRegime.charAt(0).toUpperCase() + resolvedRegime.slice(1);

  // The backend regimePlaybook is a lighter {summary,keyRisks,...} shape that lacks
  // the assetAllocation/factorPreferences/tradeSetup arrays this view maps over, so
  // only use it when it actually has that shape; otherwise use the keyed default.
  const backendPlaybook = (fullDash as any)?.regimePlaybook;
  const playbook: RegimePlaybookData | null =
    data ||
